...
export default function GlpSwap(props) {
  ...
  const tabLabel = isBuying ? t`Buy NLP` : t`Sell NLP`;
  ...
  const getError = () => {
    if (IS_NETWORK_DISABLED[chainId]) {
      if (isBuying) return [t`NLP buy disabled, pending ${getChainName(chainId)} upgrade`];
      return [t`NLP sell disabled, pending ${getChainName(chainId)} upgrade`];
    }
    ...
    if (!isBuying) {
      if (maxSellAmount && glpAmount && glpAmount.gt(maxSellAmount)) {
        return [t`Insufficient NLP balance`];
      }
  ...
  const getPrimaryText = () => {
    ...
    return isBuying ? t`Buy NLP` : t`Sell NLP`;
  };
  ...
  const buyGlp = () => {
    ...
    callContract(chainId, contract, method, params, {
      ...
      successMsg: t`${formatAmount(glpAmount, 18, 4, true)} NLP bought with ${formatAmount(
        swapAmount,
  ...
  const sellGlp = () => {
    ...
    callContract(chainId, contract, method, params, {
      ...
      successMsg: t`${formatAmount(glpAmount, 18, 4, true)} NLP sold for ${formatAmount(
        swapAmount,
        swapTokenInfo.decimals,
        4,
        true
        ...
        <div className="App-card GlpSwap-stats-card">
          <div className="App-card-title">
            <div className="App-card-title-mark">
              <div className="App-card-title-mark-icon">
                <img width="40" src={glpIcon} alt="NLP" />
              </div>
          ...
          <div className="App-card-content">
            ...
            <div className="App-card-row">
              ...
              <div className="value">
                {formatAmount(glpBalance, GLP_DECIMALS, 4, true)} NLP ($
          ...
          <div className="App-card-content">
            {!isBuying && (
              <div className="App-card-row">
                ...
                <div className="value">
                  <Tooltip
                    handle={`${formatAmount(reservedAmount, 18, 4, true)} NLP ($${formatAmount(
                      reserveAmountUsd,
                      USD_DECIMALS,
                      2,
                      true
                    ...
                    renderContent={() =>
                      t`${formatAmount(reservedAmount, 18, 4, true)} NLP have been reserved for vesting.`
                    }
                  />
                </div>
              </div>
            ...
            <div className="App-card-row">
              ...
              <div className="value">
                <Trans>
                  {formatAmount(glpSupply, GLP_DECIMALS, 4, true)} NLP ($
        ...
        <div className="GlpSwap-box App-card">
          ...
          >
            ...
            {!isBuying && (
              ...
              >
                <div className="selected-token">NLP</div>
              </BuyInputSection>
            )}
      ...
      <div className="mt-40 Tab-title-section">
        ...
        {isBuying && (
          <div className="Page-description">
            <Trans>
              Fees may vary depending on which asset you use to buy NLP. <br />
              Enter the amount of NLP you want to purchase in the order form, then check here to compare fees.
            </Trans>
          </div>
        )}
        {!isBuying && (
          <div className="Page-description">
            <Trans>
              Fees may vary depending on which asset you sell NLP for. <br />
              Enter the amount of NLP you want to redeem in the order form, then check here to compare fees.
            </Trans>
          </div>
        )}
      </div>
                  ...
                  <Tooltip
                    ...
                    renderContent={() => (
                      <p className="text-white">
                        <Trans>Available amount to deposit into NLP.</Trans>
                      </p>
                    )}
                  />
                ) : (
                  <Tooltip
                    ...
                    renderContent={() => {
                      return (
                        <p className="text-white">
                          <Trans>
                            Available amount to withdraw from NLP. Funds not utilized by current open positions.
                          </Trans>
                        </p>
                      );
                    }}
                ...
                case (isBuying && isCapReached) || (!isBuying && managedUsd?.lt(1)):
                  return (
                    <Tooltip
                      ...
                      renderContent={() => (
                        <Trans>
                          Max pool capacity reached for {tokenInfo.symbol}. Please mint NLP using another token
                        </Trans>
                      )}
                    />
                  );
